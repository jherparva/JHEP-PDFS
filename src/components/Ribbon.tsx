...
export const Ribbon = ({ onAction, activeTab, isFoliating, canInstall, onInstall }: RibbonProps) => {
  return (
    <div className="h-32 bg-white border-b border-slate-200 flex items-center px-8 gap-10 shrink-0 shadow-sm z-50">
        ...
        <div className="relative w-12 h-12 flex shrink-0 group">
           ...
           <div className="absolute inset-0 bg-gradient-to-b from-red-600 to-red-800 rounded-xl shadow-[0_2px_10px_rgba(220,38,38,0.4)] flex items-center justify-center p-[4px] transition-transform group-hover:scale-105">
              ...
              <div className="relative w-full h-full bg-slate-50 rounded-[4px] rounded-tr-[12px] flex flex-col items-center justify-center shadow-inner border border-slate-950 overflow-hidden">
                 {/* Corte de la esquina original pintado de rojo */}
                 ...
                 {/* Doblez realista esquina superior derecha (gris con borde) */}
      ...
      <div className="flex items-center h-full gap-10 flex-1">
        {actionGroups.map((group) => (
          <div key={group.name} className="flex flex-col h-full py-2.5 justify-between relative">
            <span className="text-[9.5px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1 pl-1">
              {group.name}
            </span>
              ...
              {group.tools.map((tool) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "group flex flex-col items-center justify-center p-1.5 rounded-[18px] transition-all relative min-w-[65px]",
                      active ? "bg-indigo-600 text-white shadow-lg scale-105" : "hover:bg-slate-50 text-slate-600"
                    )}
                  >
                    <div className={cn(
                      "p-3 rounded-[16px] mb-1.5 transition-all group-hover:scale-110",
                      active ? "bg-white/20" : tool.bg
                    )}>
                      <Icon size={18} strokeWidth={3} className={active ? "text-white" : tool.color} />
                    </div>
                    <span className={cn(
        ...
        <div className="flex items-center">
           {canInstall && (
             <button
               onClick={onInstall}
               className="h-16 px-6 mr-3 bg-red-50 text-red-600 font-black rounded-[20px] flex items-center justify-center gap-2 transition-all hover:bg-red-600 hover:text-white border border-red-200 uppercase tracking-widest text-[9px]"
             >
               <MonitorDown size={20} strokeWidth={3} />
               Instalar PC
             </button>
           )}
           <button
             onClick={() => onAction("save")}
             className="h-16 px-10 bg-emerald-600 text-white font-black rounded-[20px] flex items-center justify-center gap-3 shadow-xl shadow-emerald-100 transition-all hover:translate-y-[-4px] active:scale-95 border-b-4 border-emerald-900 uppercase tracking-[0.2em] text-[10px] group"
           >
             <Download size={22} strokeWidth={4} className="group-hover:scale-110 transition-transform" />
             Guardar PDF
           </button>
        </div>
      </div>
