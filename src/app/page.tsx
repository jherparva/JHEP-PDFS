...
import { SearchNumberModal } from "@/components/SearchNumberModal";
...
import { getPDFLibDocument, getPDFJSDocument } from "@/lib/pdf-cache";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const hk = (e: KeyboardEvent) => {
      ...
      else if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === "f") { e.preventDefault(); setShowSearchModal(true); }
  ...
  const handleInstallClick = async () => {
    if (deferredPrompt) {
      ...
      toast.info("Busca el ícono de una Pantallita 🖥️ arriba a la derecha en la barra de direcciones para Instalar.", { duration: 6000 });
    }
  };
  ...
  const handleAction = async (a: string) => {
    ...
    else if (a === "searchNumber") setShowSearchModal(true);
  ...
  const handleSave = async (selOnly: boolean, annos?: any[]) => {
    ...
    if (annos) {
       const task = async () => {
         for (let i = 0; i < pgs.length; i++) {
           ...
           const sourceDoc = await getPDFLibDocument(vp.editedBlob || f.blob);
           const [cp] = await pageDoc.copyPages(sourceDoc, [vp.editedBlob ? 0 : vp.pageNumber - 1]);
    ...
    const task = async () => {
      ...
      for (let i = 0; i < pgs.length; i++) {
        ...
        const sourceDoc = await getPDFLibDocument(vp.editedBlob || f.blob);
        const pageIdx = vp.editedBlob ? 0 : vp.pageNumber - 1;
        const [cp] = await doc.copyPages(sourceDoc, [pageIdx]);
    ...
    task().then(res => {
      ...
      else toast.success("PDF Guardado con éxito", { id: saveToast });
    }).catch(err => {
      console.error(err);
  ...
  const handlePrint = async () => {
    ...
    const task = async () => {
      ...
      for (const vp of pgs) {
        ...
        if (!f) continue;
        const sourceDoc = await getPDFLibDocument(vp.editedBlob || f.blob);
        const [cp] = await doc.copyPages(sourceDoc, [vp.editedBlob ? 0 : vp.pageNumber - 1]);
        if (vp.rotation % 360 !== 0) cp.setRotation(degrees(vp.rotation % 360));
        doc.addPage(cp);
      }
  ...
  const handleConvertToImages = async (format: "png" | "jpeg" | "webp") => {
    ...
    const task = async () => {
      ...
      for (const vp of list) {
        ...
        if (!f) continue;
        const sourceDoc = await getPDFLibDocument(vp.editedBlob || f.blob);
        const [cp] = await temporaryDoc.copyPages(sourceDoc, [vp.editedBlob ? 0 : vp.pageNumber - 1]);
        if (vp.rotation % 360 !== 0) cp.setRotation(degrees(vp.rotation % 360));
        temporaryDoc.addPage(cp);
      }
      
      const pdfBytes = await temporaryDoc.save();
      const pdfjsDoc = await getPDFJSDocument(new Blob([pdfBytes as any], { type: "application/pdf" }));
    ...
    const task = async () => {
      ...
      for (const doc of docsToZip) {
        ...
        for (let i = 0; i < resolvedPages.length; i++) {
          ...
          const src = await getPDFLibDocument(blob);
    ...
    const task = async () => {
      ...
      for (const doc of docsToSave) {
        ...
        for (let i = 0; i < resolvedPages.length; i++) {
          ...
          const src = await getPDFLibDocument(pageBlob);
          const safePageNum = Math.min(pageNumber, src.getPageCount() - 1);
          const [cp] = await pdfDoc.copyPages(src, [safePageNum]);
          if (rotation % 360 !== 0) cp.setRotation(degrees(rotation % 360));
          pdfDoc.addPage(cp);
  ...
  const handlePreviewDocument = async (docId: string) => {
    ...
    toast.promise(async () => {
      ...
      for (const vp of list) {
        ...
        if (!f) continue;
        const sourceDoc = await getPDFLibDocument(vp.editedBlob || f.blob);
        const [cp] = await pdfDoc.copyPages(sourceDoc, [vp.editedBlob ? 0 : vp.pageNumber - 1]);
        if (vp.rotation % 360 !== 0) cp.setRotation(degrees(vp.rotation % 360));
        pdfDoc.addPage(cp);
      }
  ...
  const getContextMenuActions = () => {
    ...
    return [
      ...
      { label: "Buscar Número...", icon: <FileSearch size={14} strokeWidth={3} />, onClick: () => setShowSearchModal(true) },
          ...
          <div className="flex flex-col flex-1 bg-white rounded-[32px] border border-slate-200 shadow-lg overflow-hidden relative">
             <div className="min-h-16 md:min-h-20 border-b border-slate-50 flex items-center justify-between px-3 md:px-8 py-2 md:py-4 bg-white/90 z-40 gap-2">
                 ...
                 <div className="hidden md:flex items-center gap-4 flex-1">
                    ...
                    {/* Botón Buscar Número & Columnas */}
                    <div className="flex items-center gap-2 ml-4">
                       <button
                         onClick={() => setShowSearchModal(true)}
                         className="h-12 px-4 bg-indigo-50 hover:bg-indigo-600 text-indigo-700 hover:text-white font-black text-[9px] uppercase tracking-wider rounded-[18px] transition-all flex items-center gap-2 border border-indigo-100 shadow-sm"
                         title="Buscar número en todas las hojas (Ctrl + F)"
                       >
                         <FileSearch size={16} strokeWidth={3} /> Buscar Número
                       </button>
                 ...
                 <div className="flex md:hidden items-center gap-2 flex-1 min-w-0">
                    ...
                    <button onClick={() => setShowSearchModal(true)} className="w-9 h-9 flex items-center justify-center bg-indigo-50 text-indigo-600 rounded-lg shrink-0" title="Buscar Número"><FileSearch size={16} /></button>
